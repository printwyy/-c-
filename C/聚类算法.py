...
df = pd.read_excel('j.xlsx', index_col=0)

#scale_values =  MinMaxScaler().fit_transform(df.values)  # 数据预处理

kmeans = KMeans(n_clusters=4).fit(df.values) # 分为 4 类
...
tsne = PCA(n_components = 2).fit_transform(df.values)
...
# 画图
plt.plot(df_1[0], df_1[1], 'bo', df_2[0], df_2[1], 'r*', df_3[0], df_3[1], 'gD',df_4[0],df_4[1],'ro')
plt.show()
